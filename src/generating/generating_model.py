...
class GeneratingModel():
    ...
    def generate_sample(self, scoring_model, physical_error_rates, num_steps=40, starting_code=None, mutate=True):
        ...
        def hard_decision(f_tensor): return (f_tensor >= 0.5).squeeze(
            0).type(torch.int16).cpu().numpy()
        bit_adj, phase_adj, check_adj = hard_decision(
            bit_adj), hard_decision(phase_adj), hard_decision(check_adj)

        if mutate and random.random() > params['p_skip_mutation']:
            self.mutate(
    ...
    def mutate(self, bit_adj, phase_adj, check_adj):
        ...
        according to `p_random_mutation_range`
        """
        p_edge_flip = np.random.uniform(
            low=params['p_random_mutation_range'][0], high=params['p_random_mutation_range'][1])
        ...
        for check_1 in range(check_adj.shape[0]):
            for check_2 in range(check_1):
                if random.random() < p_edge_flip:
                    check_adj[check_1, check_2] = 1 - \
                        check_adj[check_1, check_2]
                    check_adj[check_2, check_1] = 1 - \
                        check_adj[check_2, check_1]
