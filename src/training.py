...
def initialize_scoring_model(device, plot_loss=None, scoring_model=None, initialize_epoch_start=1):
    ...
    scoring.score_training.main_training_loop(
        "initialization", model, ge, gc, utils.get_best_scoring_model_path(), params['n_score_training_per_epoch_initial'], plot_loss, epoch_start=initialize_epoch_start)
    return model
...
def train_score_model_with_generator(genetic_epoch, scoring_model: score_model.ScoringTransformer, scoring_model_copy: score_model.ScoringTransformer,
    ...
    scoring.score_training.main_training_loop(f"gen_epoch_{genetic_epoch}",
                                              scoring_model, ge, gc, utils.get_best_scoring_model_path(), params['n_score_training_per_epoch_genetic'],  plot_loss)
...
def main(plot_loss=None, load_saved_scoring_model=False, load_saved_generating_model=False, skip_initialization_training=False, skip_eval=False, initialize_epoch_start=1):
    ...
    for genetic_epoch in range(params['n_genetic_epochs']):
        if not skip_eval:
            evaluate_performance(scoring_model, generating_model, p_eval_range, genetic_epoch, True and initialize_epoch_start == 1)
        ...
        del scoring_copied
    if not skip_eval:
        evaluate_performance(scoring_model, generating_model, p_eval_range, genetic_epoch, False)
